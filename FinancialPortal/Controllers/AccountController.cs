...
namespace FinancialPortal.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        [AllowAnonymous]
        [Route("UsersForHousehold")]
        public User[] UsersForHousehold(string household)
        {
            var repo = HttpContext.Current.GetOwinContext().Get<SqlConnection>().As<IUserDataAccess>();
            var h = new Guid(household);

            IList<User> i = repo.GetUsersByHousehold(h);
            User[] s = new User[i.Count];
            int counter = 0;
            foreach (User a in i)
            {
                s[counter] = a;
        ...
        // POST api/Account/SetLoginModel
        //[Route("SetLoginModel")]
        //public async Task<IHttpActionResult> SetLoginModel(string token)
       // {

       //     AddExternalLoginBindingModel t = new AddExternalLoginBindingModel();
        //    t.ExternalAccessToken = token;
        //    return await AddExternalLogin(t);
       // }
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            User u=UserManager.FindByEmail(user.Email);
        ...
        [AllowAnonymous]
        [Route("AssignUniqueHousehold")]
        public string AssignUniqueHousehold(int userid)
        {
            Guid guid = Guid.NewGuid();
            var repo = HttpContext.Current.GetOwinContext().Get<SqlConnection>().As<IUserDataAccess>();
            User u=repo.SelectUser(userid);
            u.Household=guid;
            repo.UpdateUser(u);
        ...
        public void LeaveHousehold(int userid)
        {
            Guid guid = new Guid(userid+100, 0, 0, new byte[8]);
            var repo = HttpContext.Current.GetOwinContext().Get<SqlConnection>().As<IUserDataAccess>();
            User u = repo.SelectUser(userid);
            u.Household = guid;
            repo.UpdateUser(u);
